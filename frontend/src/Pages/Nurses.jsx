...
export default function Nurses() {
  ...
  const handleAddEditNurse = async () => {
    ...
    if (editingNurse) {
      ...
      const response = await fetchRequest(
        `${import.meta.env.VITE_APP_SERVER_URI}user/nurse/${editingNurse.empId}`,
        "PUT",
        formData
      );
      ...
      // Update nurse in local state
      setNurses((prev) =>
        prev.map((n) =>
          n.id === editingNurse.id ? { ...formData, id: n.id } : n
        )
      );
    } else {
      // Add new nurse
    ...
    setFormData({ name: "", position: "", registered: false, ssn: "" });
  };
  ...
  useEffect(() => {
    fetchRequest(`${import.meta.env.VITE_APP_SERVER_URI}user/nurse`, "GET").then(
      (response) => {
        if (!response.success) {
          toast.error(response.error);
          return;
          ...
          <div className="fixed inset-0 backdrop-blur-sm flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full relative">
              ...
              <input
                type="text"
                name="name"
                // disabled={editingNurse ? true : false}
                placeholder="Name"
                value={formData.name}
